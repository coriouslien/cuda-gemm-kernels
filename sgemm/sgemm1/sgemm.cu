                                                                                        ...
                                                                                        Alpha alpha, Beta beta)
{
    ...
    // using R2G_Atom = Copy_Atom<UniversalCopy<uint128_t>, half_t>;
    // using R2G_Atom = Copy_Atom<UniversalCopy<uint64_t>, half_t>;
    using R2G_Atom = Copy_Atom<AutoVectorizingCopyWithAssumedAlignment<128>, half_t>;
    ...
    Tensor tCgC = thr_mma.partition_C(gC); // (MMA,MMA_M,MMA_N)
                                           // Print shape and strides for thread 0
             ...
             cudaStream_t stream = 0)
{
    ...
    auto dC = make_stride(Int<1>{}, ldC); // (dM, dN) // column-major
             ...
             cudaStream_t stream = 0)
{
    ...
    auto dC = make_stride(ldC, Int<1>{}); // row-major
  ...
  print_latex(mmaC);
    ...
    dim3 dimGrid(size(ceil_div(M, bM)),
                 size(ceil_div(N, bN)));
    gemm_device<<<dimGrid, dimBlock, smem_size, stream>>>(prob_shape, cta_tiler,
...
void verify_with_cublas(
    ...
    cute::half_t *A, cute::half_t *B, cute::half_t *C_cute,
    int ldA, int ldB, int ldC)
{
    cute::half_t *C_ref;
    cudaMalloc(&C_ref, M * N * sizeof(cute::half_t));
    cudaMemset(C_ref, 0, M * N * sizeof(cute::half_t));
    ...
    cublasGemmEx(
        ...
        M, N, K,     // M, N, K in correct order
        &alpha,
        A, CUDA_R_16F, ldA,
        B, CUDA_R_16F, ldB,
        &beta,
        C_ref, CUDA_R_16F, ldC,
        CUBLAS_COMPUTE_32F,
        CUBLAS_GEMM_DEFAULT_TENSOR_OP);

    // Copy both results to host
    std::vector<cute::half_t> h_cute(M * N);
    std::vector<cute::half_t> h_ref(M * N);
    cudaMemcpy(h_cute.data(), C_cute, M * N * sizeof(cute::half_t), cudaMemcpyDeviceToHost);
    cudaMemcpy(h_ref.data(), C_ref, M * N * sizeof(cute::half_t), cudaMemcpyDeviceToHost);

    // Print first 8 elements of both
    printf("cuBLAS C_ref[0..7]: ");
    for (int i = 0; i < 8; i++)
    {
        printf("%.3f ", (float)h_ref[i]);
    }
    printf("\n");
    ...
    for (int i = 0; i < 8; i++)
    {
        printf("%.3f ", (float)h_cute[i]);
    }
    printf("\n");
    // Print as 2D to see row vs column ordering
    printf("cuBLAS C_ref as 2D (first 4 rows, 4 cols):\n");
    for (int row = 0; row < 4; row++)
    {
        for (int col = 0; col < 4; col++)
        {
            // column-major: index = row + col * M
            printf("%.3f   ", (float)h_ref[row + col * M]);
        }
        printf("\n");
    }
    printf("\n");
    printf("CUTE C as 2D (first 4 rows, 4 cols):\n");
    for (int row = 0; row < 4; row++)
    {
        for (int col = 0; col < 4; col++)
        {
            printf("%.3f   ", (float)h_cute[row + col * M]);
        }
        printf("\n");
    }
    ...
    for (int row = 0; row < M; row++)
    {
        ...
        {
            // // cuBLAS stores C column-major: C[row,col] at row + col*M
            // float ref_val = (float)h_ref[row + col * M];
            // // Your kernel stores C^T column-major: C[row,col] at col + row*M
            // float cute_val = (float)h_cute[col + row * M];

            float ref_val = (float)h_ref[row + col * M];
            float cute_val = (float)h_cute[row + col * M];

            float diff = abs(cute_val - ref_val);
            max_error = max(max_error, diff);
        }
    }
    printf("Max error vs cuBLAS: %f %s\n",
           max_error,
           max_error < 1.0f ? "PASSED" : "FAILED");
...
int main(int argc, char **argv)
{
    ...
    verify_with_cublas(
        ...
        ldA, ldB, ldC);
    auto C = thrust::raw_pointer_cast(d_C.data());
    printf("C pointer:          %p\n", (void *)C);
    printf("d_C.data() pointer: %p\n", (void *)thrust::raw_pointer_cast(d_C.data()));
